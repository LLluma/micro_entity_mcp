...
def test_create_defaults_status_and_order(tmp_path: Path) -> None:
    ...
    r = asyncio.run(go())
    assert set(r.structured_content.keys()) == {"item", "commit", "progress"}
...
def test_create_order_increments(tmp_path: Path) -> None:
    ...
    assert set(first.keys()) == {"item", "commit", "progress"}
    assert set(second.keys()) == {"item", "commit", "progress"}
...
def test_create_returns_wrapped_dict(tmp_path: Path) -> None:
    ...
    # Top-level keys: "item" plus additive "commit" and "progress"
    assert set(data.keys()) == {"item", "commit", "progress"}
